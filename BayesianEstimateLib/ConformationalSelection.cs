...
{
    /// <summary>
    /// this is the basic conformational selection model, without kM or offset. also in this model, the R0_B, R0_B_star, R0_AB and R0_AB_star are all determined by the end of attaching phase.
    /// it states that there are two different conformations for the free antibody and these two are in a steady states
    /// B->B*      : ka1, kd1
    /// A+B*->AB*  : ka2, kd2
    /// 
    /// we have ka1,ka2, kd1,kd2
    /// different equations
    ///  
    ///  dAB*/dt=ka2xAxB*-kd2*AB*
    ///  dB/dt=-ka1B +kd1B*
    ///  
    ///  Bmax=B+B*+AB*
    ///  
    /// </summary>
    public class ConformationalSelection:SimulationSPR 
    {
        ...
        protected double _R0_AB_Star;//<<=== in this case, SSPR_r0 is the same as R0_AB_star
        ...
        public ConformationalSelection()
            : base()
        {
        ...
        ///     the order is 0-ka1, 1-kd1, 2-ka2, 3-kd2,  4-conc, 5-Rmax, 
        ///     6-R0_AB_Star, 7-R0_B, 
        ///     8-attachDuration, 9-detachDuration,10-deltaT
        ///     ka1 and kd1 is the conformational change and ka2 and kd2 are the complex binding reaction 
        ///     B* is the active conformation that can bind to ligand
        /// </param>
        public ConformationalSelection(double[] _params):base()
        {
        ...
        /// 
        /// it has the order, the order is 0-ka1, 1-kd1, 2-ka2, 3-kd2, 4-conc, 5-Rmax, 
        ///     6-R0_AB_Star, 7-R0_B,
        ///     8-attachDuration, 9-detachDuration,10-deltaT
        ...
        ///    B* is the active conformatoin that can bind to ligand. 
        /// </summary>
        /// <param name="_params">it has the order, the order is 0-ka1, 1-kd1, 2-ka2, 3-kd2, 4-conc, 5-Rmax, 
        ///     6-R0_AB_Star, 7-R0_B, 
        ///     8-attachDuration, 9-detachDuration,10-deltaT
        ///     So far DON"T have kM in there now.
        ///     here we also allow variable length of params, but only
        ...
        public override void setParameters(double[] _params)
        {
            ...
            if (_params.Count() < 6) //8 is the minimum number of parameters to be set. since we are doing non-offset model. we can do this.
            {
                throw new Exception("the input parameter array is not valid. doesn't contain enough elements");
            }
            this._ka = _params[0];
            ...
            this._R0_AB_Star = this.SSPR_r0;
            if (_params.Count() >= 7 && _params[6] > 0)
            {
                //R0_AB = true;
                this.SSPR_r0 = _params[6];
                this._R0_AB_Star = _params[6];
            }
 /*           
            this._R0_AB_Star = -1;
            ...
            {
                this._R0_B = _params[7];
            }
            this._R0_B_Star = -1;
            if (this._R0_AB_Star != -1 && this._R0_B != -1)
            {
                this._R0_B_Star = this._Rmax - this._R0_B - this._R0_AB_Star;
            }
            //if (_params.Count() >= 9 && _params[8] > 0)
            //{
                //this._R0_B_Star = _params[8];
            //}
        ...
        public override void run_Attach()//this is the Euler
        {
            ...
            //double dR_AB_per_dt;
            double dR_B_per_dt;// dR_B_per_dt;
            double dt;
            for( int i=0; ;i++)
            {
                double R_B_Star_i = _Rmax - _R_B_Att[i]  - _R_AB_Star_Att[i];
	            //dR_AB_per_dt=_ka*_conc*R_B_i
                //                        - _kd * _R_AB_Star_Att[i];// +_kd2 * _R_AB_Att[i] - _ka2 * _R_AB_Star_Att[i];
                dR_AB_Star_per_dt = _ka2 * _conc*R_B_Star_i - _kd2 * _R_AB_Star_Att[i];

                //dR_B_per_dt = -1 * dR_AB_per_dt - _ka3 * R_B_i + _kd3 * _R_B_Star_Att[i];
                dR_B_per_dt = -1* _ka * _R_B_Att[i] + _kd * R_B_Star_i;
                ...
                _R_B_Star_Att[i + 1] = _Rmax - _R_B_Att[i + 1] - _R_AB_Star_Att[i + 1];
                _ru_attach[i + 1] = _R_AB_Star_Att[i+1];
        ...
        public  void run_Attach_RK()//this is the Runge-Kutta
        {
            ...
            for (int i = 0; i < _R_AB_Star_Att.Count(); i++)
            {
                RABs.Add(new List<double>{_R_AB_Star_Att[i], _R_B_Star_Att[i], _R_B_Att[i]});
            }
            ...
            for (int i = 1; i < RABs.Count; i++)
            {
                ...
                this._R_B_Star_Att[i] = RABs[i][1];
                this._R_B_Att[i] = RABs[i][2];
                this._ru_attach[i] = _R_AB_Star_Att[i];
            }
            //done
        }
        ...
        protected List<double> Derivative_Attach(double _t, List<double> _RABs)
        {
            ...
            //in this _RABs, 0 is for R_AB_Star and  1 for R_B_Star, 2 for R_B
            //double R_B_i = _Rmax - _RABs[0] - _RABs[1] - _RABs[2];
            double dR_AB_Star_per_dt = _ka2 * _conc * _RABs[1]
                                        - _kd2 * _RABs[0];// +_kd2 * _RABs[1] - _ka2 * _RABs[0];
            //double dR_AB_per_dt = _ka *_conc* _RABs[3] - _kd * _RABs[1];

            double dR_B_per_dt = -1*_ka * _RABs[2] + _kd * _RABs[1];

            nextRABs.Add(dR_AB_Star_per_dt);
            //nextRABs.Add(dR_AB_per_dt);
            nextRABs.Add(0-dR_B_per_dt-dR_AB_Star_per_dt); //the fact is that dB+dB*+dAB+dAB*=0; all changes sum to zero.
        ...
        public override void run_Detach() //Euler scheme
        {
            ...
            if (SSPR_r0 < 0 && _R0_AB_Star < 0)
            {
                ...
                Console.Write("AB_Star0:" + this._R_AB_Star_Det[0] + "\n");
            }
            else
            {
            ...
            if (this._R0_B < 0 && _R0_B_Star < 0)
            {
                ...
                Console.Write("RB0:" + this._R_B_Det[0] + "RB_star:" + this._R_B_Star_Det[0] + "\n");
            }
            else
            {
            ...
            for (int i = 0; ; i++)
            {
                double R_B_Star_i = _Rmax - _R_B_Det[i]  - _R_AB_Star_Det[i];
                ...
                dR_B_per_dt = -1* _ka * _R_B_Det[i] + _kd * R_B_Star_i;
                ...
                _R_B_Det[i + 1] = _Rmax - _R_B_Star_Det[i + 1]  - _R_AB_Star_Det[i + 1];
                _ru_detach[i + 1] =  _R_AB_Star_Det[i + 1];
        ...
        public void run_Detach_RK() //Runge-Kutta scheme
        {
            ...
            List<List<double>> RABs = new List<List<double>>();
            for (int i = 0; i < _R_AB_Star_Det.Count(); i++)
            {
                RABs.Add(new List<double> { _R_AB_Star_Det[i],  _R_B_Star_Det[i], _R_B_Det[i] });
            }
            ...
            for (int i = 1; i < RABs.Count; i++)
            {
                ...
                this._R_B_Star_Det[i] = RABs[i][1];
                this._R_B_Det[i] = RABs[i][2];
                this._ru_detach[i] =  _R_AB_Star_Det[i];
            }
            //done!!
        }
        ...
        protected List<double> Derivative_Detach(double _t, List<double> _RABs)
        {
            ...
            nextRABs.Add(dR_AB_Star_per_dt);
            //nextRABs.Add(dR_AB_per_dt);
            nextRABs.Add(0  - dR_AB_Star_per_dt - dR_B_per_dt);
            nextRABs.Add(dR_B_per_dt);
